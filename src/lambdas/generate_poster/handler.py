...
from src.common import config, aws, adobe
...
def handler(event: Dict[str, Any], context: object) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.append_keys(sku=sku) # Add SKU to all subsequent logs for this invocation
        logger.info("Processing poster generation request.", extra={"row_data": row_data})
    except (KeyError, ValueError) as e:
        logger.warning("Incoming event is missing or has malformed data.", extra={"error": str(e)})
        return {"statusCode": 400, "body": json.dumps({"error": f"Invalid event data: {e}"})}
    ...
    if not adobe_creds_str:
        ...
        return {"statusCode": 500, "body": json.dumps({"error": "Could not fetch Adobe credentials"})}
    ...
    try:
        ...
        return {"statusCode": 500, "body": json.dumps({"error": "Malformed Adobe credentials secret"})}
    ...
    template_url = aws.create_presigned_url(assets_bucket, f"templates/{template_name}", method="GET")
    output_url = aws.create_presigned_url(outputs_bucket, f"generated/{output_name}", method="PUT")

    if not template_url or not output_url:
        logger.error("Failed to create one or more S3 pre-signed URLs.")
        return {"statusCode": 500, "body": json.dumps({"error": "Failed to generate S3 URLs"})}
    ...
    if not job_status_url:
        ...
        return {"statusCode": 502, "body": json.dumps({"error": "Bad Gateway: Adobe API job submission failed"})}

    # 5. Return the job status URL to the Inngest orchestrator
    # Inngest will use this URL in a `step.sleep()` and polling loop.
    logger.info("Successfully submitted job to Adobe. Returning status URL to orchestrator.")
    return {
