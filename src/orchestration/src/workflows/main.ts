...
const ADOBE_CONCURRENCY_LIMIT = parseInt(
  process.env.ADOBE_CONCURRENCY_LIMIT || "200",
  10,
);
...
export const fanOutAndGenerate = inngest.createFunction(
  ...
  async (ctx) => {
    ...
    const readResult = await step.run("1-read-google-sheet", async () => {
      ...
      if (statusCode !== 200) {
        throw new Error(
          `Read sheet Lambda failed: status=${statusCode} body=${JSON.stringify(body)}`,
        );
      }
      return body as {
...
export const generatePoster = inngest.createFunction(
  ...
  async (ctx) => {
    ...
    const adobeCreds = await step.run(
      "get-adobe-creds-for-polling",
      async () => {
    ...
    while (adobeJobStatus === "running" || adobeJobStatus === "unstarted") {
      ...
      jobDetails = await step.run("5-poll-adobe-status", async () => {
        const res = await axios.get(submissionResult.job_status_url, {
    ...
    if (adobeJobStatus === "succeeded") {
      await step.sendEvent("6-send-success-result", {
        ...
        data: {
          ...
          sku,
          status: "SUCCESS",
          output_key: submissionResult.output_key,
        },
      });
    } else {
      await step.sendEvent("6-send-failure-result", {
        ...
        data: {
          ...
          sku,
          status: "FAILURE",
          error: jobDetails?.errors?.[0]?.title ?? "Unknown Adobe failure",
        },
      });
...
export const aggregateAndReport = inngest.createFunction(
  ...
  async (ctx) => {
    const { event, step } = ctx as any;
    const { run_id, total_jobs, invalid_rows_count, spreadsheet_id } =
      event.data as {
        run_id: string;
        total_jobs: number;
        invalid_rows_count: number;
    ...
    const successful_jobs = results
      .map((e) => e?.data)
      .filter((d) => d?.status === "SUCCESS");
    const failed_jobs = results
      .map((e) => e?.data)
